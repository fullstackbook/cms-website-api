...
async function main() {
  ...
  for (let machine of data) {
    console.log("update " + JSON.stringify(machine));
    const res = await fetch("http://127.0.0.1:1337/api/flying-machines/" + machine.id, {
      method: "PUT",
      body: JSON.stringify({
        data: {
          Name: machine.attributes.Name,
          Description: machine.attributes.Description,
          Image: imgId,
